...
export function BoardView({ rows, onOpen }: { rows: TaskListRow[]; onOpen: (id: string) => void }) {
  ...
  return (
    <div style={{
      display: 'grid', gap: 12,
      // Columns on a desktop; a single stack on a phone, where three columns of 110px are unreadable.
      ...
      {columns.map(col => {
        ...
        return (
          <section key={col.status} style={{ display: 'grid', gap: 10, minWidth: 0 }}>
            <header style={{
            ...
            {col.rows.length === 0
              ? (
                <div style={{
                  ...surface, padding: 14, fontSize: 11.5, color: 'var(--text-tertiary)',
                  borderStyle: 'dashed',
                }}>
                  Nothing here.
                </div>
